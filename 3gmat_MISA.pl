#!/usr/bin/perl -w
# Author: Thomas Thiel
# Program name: misa_mdf.pl

##   Note: Modified by Manosh (2010) to retrieve SSR conation sequences to the corresponding SSR motif
##         Only can use for Chromosome or Scaffold sequences mining 
...
###Modified by Manosh (2010 October)
## _______________________________________________________________________________
##
...
open (IN,"<$ARGV[0]") || die ("\nError: FASTA file doesn't exist !\n\n");
 ...
 print OUT "Seq_ID\tSSR nr.\tSSR type\tSSR\tsize\tOnChr_start\tOnChr_end\t";
 print OUT "SSR_srial_no\tSSR_with400bp_flSeq \tSSR_seq_whole\tSSR_on_New_start\tSSR_on_New_end\t"; ## manosh modify 2013 OCTOBER
 print OUT "SSR_Class\tSSR_motif_Rich_in \n";  ## manosh modify 2013 OCTOBER
...
###   Manosh modify to get flanking primer sequences Model::  --<--200 bp----<-||  SSR  ||-->--200 bp--->	
#----------------------------------------------------------------------------------------------------------	
	if ($start < 200 ) {
		$cut_start = 0;
		}
	...
	$cut_end = ($end - $start + 401);
	$flseq = substr ($seq, $cut_start, $cut_end);

	$ssr_size_f_cout = ($end - $start+1);
	$SSR_seq_whole = substr ($seq, $start-1, $ssr_size_f_cout);

	
##	Manosh::  Find SSR Position in cut sequences
#----------------------------------------------------------------------------------------

while($flseq =~ /$SSR_seq_whole/ig){
		$position = pos($flseq)
					};
$nwpos1 = ($position- length ($SSR_seq_whole) +1 );
if ($nwpos1 > 251) { $nwpos1 = 200
					};
 $manosh=$nwpos1+length ($SSR_seq_whole);

if ($position > $manosh ) {$nwpos2=$manosh}
else {$nwpos2 =$position};

## code for SSR serial number generate-------> Start code
	$slcount++;
	$f_slcount = sprintf("%06d", $slcount);
	$genotypename= substr $id, 0, 5;
	 

## code for SSR clasification [ ClassI >= 20] and [classII <20]  ------> 
   $ma_SSR_size = ($end - $start + 1);
   if ($ma_SSR_size <20 ) { $maSSRclass = "Class II"; $countCII++;
                             }
	else {$maSSRclass = "Class I"; $countCI++ };

## Code for  SSR types AT or GC rich
$Seq_ma_SSR_motif = $SSR_seq_whole;
$count_A = () = $Seq_ma_SSR_motif  =~ /A/ig;  # count the number of  A.
$count_T = () = $Seq_ma_SSR_motif   =~ /T/ig; # count the number of  T.
$count_G = () = $Seq_ma_SSR_motif   =~ /G/ig; # count the number of  G.
$count_C = () = $Seq_ma_SSR_motif   =~ /C/ig; # count the number of  C.


$count_AT = $count_A + $count_T;
$count_GC = $count_G + $count_C;

 if ($count_AT > $count_GC) {
        $ma_AT_GC= "AT rich SSR"; $ATrich++;
    } elsif ($count_AT < $count_GC) {
        $ma_AT_GC= "GC rich SSR"; $GCrich++;
    } elsif ($count_AT = $count_GC) {
        $ma_AT_GC= "AT/GC Balance SSR"; $AT_GC_bal++
    };

##------

	print OUT "$id\t$count_seq\t$ssrtype\t$ssrseq\t",($end - $start + 1),"\t$start\t$end\t$genotypename"."SSR"."$f_slcount\t$flseq\t$SSR_seq_whole\t$nwpos1\t$nwpos2\t$maSSRclass\t"; #t$flseq\n" = retrive SSR containg region with around 400 bp seq {manosh modify}#
    print OUT "$ma_AT_GC\n";
	};
...
print OUT "SSR density (1 ssr per ** bp):\t", $size_sequences/$total,"\n";
print OUT "ClassII SSR:\t$countCII\n";
print OUT "ClassI SSR:\t$countCI\n";
print OUT " ClassI+ClassII (note: Compund SSR considered as one SSR):\t ", $countCII+$countCI, "\n\n\n";

print OUT "AT rich SSR:\t $ATrich\n";
print OUT "GC rich SSR:\t $GCrich\n";
print OUT "AT/GC balance:\t $AT_GC_bal\n";

# show analysis status on the screan  during SSR search
#print "\t Successfully SSR Search complite\n ------------------------------------------\n"; 
 print "$number_sequences Seq search, $total SSR Found,";
